...
```bash
npx skills add utarn/engineer-skills --skill=diagnosing-bugs
```
...
[Source](https://github.com/utarn/engineer-skills/tree/main/skills/engineering/diagnosing-bugs)
...
Reach for it on the hard ones: the bug that resists a first glance, the intermittent flake, the regression that crept in between two known-good states. For a quick throwaway to sanity-check a design question rather than chase a defect, use [prototype](https://github.com/utarn/engineer-skills/tree/main/skills/engineering/prototype/SKILL.md) instead.
...
`diagnosing-bugs` is a reach-for-it-anytime standalone — you drop into it the moment something is broken, and drop out once the fix and its regression test are in. Its post-mortem hands off to [improve-codebase-architecture](https://github.com/utarn/engineer-skills/tree/main/skills/engineering/improve-codebase-architecture/SKILL.md) when the real finding is that there's no good seam to lock the bug down — the code, not the bug, is the problem. When you're unsure which skill fits, [ask-utarn](https://github.com/utarn/engineer-skills/tree/main/skills/engineering/ask-utarn/SKILL.md) routes you.
