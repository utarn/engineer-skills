...
- **Hard dependency** (`to-issues`, `to-issues-yes`, `to-prd`, `to-tickets`, `to-spec`, `triage`) — include an explicit one-liner: _"… should have been provided to you — run `/setup-utarn-skills` if not."_ Without the mapping, output is wrong, not just fuzzy.
